...
export default {
  ...
  created() {
    ...
    if (Cookie.getCookie("token") != "" && Cookie.getCookie("token") != null) {
      var socket;
      if (typeof WebSocket == "undefined") {
        ...
        socket = new WebSocket(
          baseUrl.replace("http", "ws") +
            "/websocket/" +
            _this.$store.state.repairorder.repairorderId
        );
  ...
  methods: {
    ...
    fasong() {
      ...
      var datalist = {
        userId: this.$store.state.repairorder.repairorderId,
        userName: this.$store.state.repairorder.repaiName,
        phone: this.$store.state.repairorder.repaiPhone,
        userType: 1,
