<template>
  <div style="position:relative" class="xiongxiong1">
    ...
    <li @click="xuanzhe(0)">
      ...
      <li v-show="isyuangong1 && isyuangong && this.$store.state.repairorder.customerId !='' ">
        <li v-for="(list,index) in list" :key="index">
        <span>客户：{{list.customerName}}</span>
      </li>
      <li v-show="isyuangong1 && isyuangong && this.$store.state.repairorder.customerId !='' ">
          <li v-for="(list,index) in list" :key="index">
        <span>终端客户级别：{{list.customerLevel}}</span>
      ...
      <li v-show="isyuangong && this.$store.state.repairorder.customerId =='1'">
        <span>终端客户级别：</span>
...
export default {
  ...
  created() {
    ...
    this.$Axios
      .post(
        baseUrl +
          "/applyJob/selectApplyJobAndProduct " ,id
      )
      .then(function(response) {
        //重复点击

        if (response.data.code == 200) {
       let  productLine2=response.data.data.productInfoList;
      
           _this.list = productLine2;
  ...
  methods: {
    ...
    totosend() {
      ...
      for (let i in this.restaurants) {
        ...
        arrstr.push({ contacts: this.restaurants[i].value, phone: this.restaurants[i].phone, }); }

      data.customer_level_id = this.Customer.customerLevel;
      data.name = this.Customer.customerName;
      data.contact = this.Customer.contacts;
      data.contactPhone = this.Customer.phone;
      data.productLine =this.Customer.productLine;
      data.customerLevel =this.Customer.customerLevel;
      data.ApplicationType=this.Customer.ApplicationType;
      console.log(data.ApplicationType)
      ...
      data.accessories = this.accessories;
      data.isshifu = this.bool01;
      this.Customer.id = this.$route.query.id1
      storage.setItem("staffClassList", JSON.stringify(data));

      if (Cookie.getCookie("avatar") == 2) {
        _this.$router.push({
          path: "/repairconfirm-chongxintibao",
          query: { id: 2,list:this.list,leixing:this.$route.query.leixing,id2:this.$route.query.id1}
        });
      } else if (Cookie.getCookie("avatar") == "1") {
        _this.$router.push({
          path: "/repairconfirm-chongxintibao",
