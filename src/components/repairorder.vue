<template>
  <div style="position:relative" class="xiongxiong1">
    ...
    <ul class="main">
      ...
      <li>
        <span>序列号：</span>
        <input type="text" placeholder maxlength="50" v-model="DeviceNumber" style="width:3.6rem" />
        <u class="jiance1" @click="See"></u>
        <u class="jiance" @click="isproductLine"></u>
      </li>
      <li>
        <div class="xuhao">
          <ol>
            <li
              v-for="(numlist_class,i_num) in this.$store.state.repairorder.ClassList"
              :key="i_num"
            >
              {{numlist_class.name}}
              <span @click="deviceNumber_minus_class(i_num)"></span>
            </li>
          </ol>
        </div>
      </li>
      <li>
        <p>产品线：{{this.$store.state.repairorder.productLine}}</p>
        <!-- <i></i> -->
      </li>
      <li>
        <p>设备机型：</p>
        <span class="miaosu">
          <em
            v-for="(modelist,modenum) in this.$store.state.repairorder.DeviceType"
            :key="modenum"
          >{{modelist.name}}</em>
        </span>
        <!-- <i></i> -->
      </li>
      <li v-show="this.$store.state.repairorder.ApplicationType =='维修' ">

        <!-- <p>故障类型：{{this.$store.state.repairorder.modelNumber}}</p>
      
        <i></i> -->
        <span>故障类型：</span>
  <el-select  style="width:230px;"  v-model="Customer.modelNumber"
          placeholder="请选择"
          clearable
        >
          <el-option
            v-for="item in modelNumberList"
            :key="item.id"
            :label="item.lable"
            :value="item.value"
          ></el-option>
        ...
        <span>客户：{{this.$store.state.repairorder.name}}</span>
      </li> -->
        <!-- <input type="text" placeholder maxlength="50" v-model="Customer.customerName" /> -->
        <!-- <i v-show="!Customershow"></i> -->
        <!-- <el-button  style="width:90px;height:40px;background-color: #9180CA;position: absolute;top:10px;right:30px;" type="primary" icon="el-icon-search" @click="search1">搜索</el-button> -->
       <!-- <select v-model="productLine"  style="width:200px;height:30px;position: absolute;margin-top:18px;border-radius:4px; " @click="bailing(productLine)">
         <option :value="productcont.value" v-for="(productcont,productindex) in user_list"  :key="productindex">{{productcont.lable}}</option>
       </select> -->
        <!-- <el-select style="width:300px;height:30px;position: absolute;margin-top:12px;border-radius:4px; "  v-model="Customer.customerName"  clearable   filterable placeholder="请选择">
        <el-option v-for="(productcont,productindex) in user_list" :key="productindex"  :value="productcont.customerName"> </el-option>
        </el-select> -->
 <!-- <ul
      class="list"
    ...
    >   
      ...
      <!-- </li>
        ...
        <!-- <u v-show="Customershow" class="jiance" @click="addCustomerinfo"></u> -->

      <!-- <li v-show="isyuangong1 && isyuangong && this.$store.state.repairorder.customerId !='' ">
        <span>终端客户级别：</span>
        <el-select
 ...
 <li v-show="isyuangong1 && isyuangong && this.$store.state.repairorder.customerId !='' ">
      ...
      <li v-show="isyuangong && this.$store.state.repairorder.customerId !='' ">
        <span>电话：</span>
        <input type="text" placeholder maxlength="11" v-model="Customer.phone" />
        <i></i>
      </li>
      ...
      <li v-show="isyuangong && this.$store.state.repairorder.customerId ==''">
        <span>终端客户级别：</span>
        <el-select v-model="Customer.customerLevel" placeholder="请选择" clearable disabled>
          <el-option
            v-for="item in customerLevels"
            :key="item.id"
            :label="item.lable"
      ...
      <li v-show="isyuangong == false && isyuangong2">
        <span>客户：{{this.$store.state.repairorder.name}}</span>
      </li>
      <li v-show="isyuangong == false  &&  isyuangong2">
        <span>终端客户级别：</span>
      ...
      <!-- <li
        @click="xuanzhe(3)"
        v-show="isyuangong && this.$store.state.repairorder.ApplicationType =='维修' "
      >
        <p>是否需要更改配件</p>
        <i></i>
      </li> -->
      <li
        v-show="bool01 == 1 && isyuangong && this.$store.state.repairorder.ApplicationType =='维修'"
      >
        <span>配件名称：</span>
        <input type="text" placeholder="配件名称" v-model="accessories" />
      </li>
      <li
        v-show="bool01 == 1 && isyuangong && this.$store.state.repairorder.ApplicationType =='维修'"
      >
      ...
      <li v-show="this.$store.state.repairorder.ApplicationType =='维修'">
        <textarea type="text" placeholder="故障描述" v-model="repairWhy" maxlength="130"></textarea>
      </li>
      <li v-show="this.$store.state.repairorder.ApplicationType =='维修'">
        <textarea type="text" placeholder="服务内容" v-model="serviceContent"></textarea>
      </li>
      <li v-show="this.$store.state.repairorder.ApplicationType =='维修' && isyuangong">
        <textarea type="text" placeholder="初步解决方法" v-model="solution"></textarea>
      </li>
    </ul>

    <!-- <textarea class="baoxiuyuanyin" placeholder="请输入报修原因" v-model="repairWhy" maxlength="130"></textarea> -->
    <div
      style="font-size:.24rem;color:red;text-align: center;margin: .2rem 0;  "
      v-show="this.$store.state.repairorder.ApplicationType =='维修'"
    >附带机器故障图片，最多上传三张故障图片，且大小不能超过8M</div>
    <div style="margin:.4rem .4rem" v-show="this.$store.state.repairorder.ApplicationType =='维修'">
      <el-upload
        :multiple="multiple"
        :headers="{token:token}"
        :action="baseUrl1+'/upload/image'"
        :file-list="imglistlist"
...
export default {
  ...
  created() {
    ...
    this.$Axios
      .post(
        baseUrl +
          "/dicCode/queryDicByMoreType?type=APPLY_TYPE,PRODUCT_LINE,CUSTOMER_LEVEL"
      )
      .then(function(response) {
        //重复点击
  ...
  methods: {
    ...
    typelist() {
      this.$Axios
        .post(
          baseUrl +
            "/dicCode/queryAdminDicByType?type=DEPARTMENT,USERTYPE,PROVINCE,CUSTOMER_SERVICE_LEVEL,CUSTOMER_TYPE,CUSTOMER_LEVEL,COUNTRY,CITY,FAULT_TYPE"//bailing  MODELNUMBER
        )
        .then(res => {
          ...
          if (res.data.code == 200) {
            ...
            let modelNumberList1;
                modelNumberList1 = res.data.data.FAULT_TYPE;
                this.modelNumberList=modelNumberList1;
   ...
   search1() {
    ...
    totosend() {
      ...
      data.contact = this.Customer.contacts;
      data.contactPhone = this.Customer.phone;
      data.modelNumber =this.Customer.modelNumber;
      ...
      if (Cookie.getCookie("avatar") == 2) {
        _this.$router.push({
          path: "/repairconfirm",
          query: { id: 2,modelNumber:this.Customer.modelNumber }
        });
      } else if (Cookie.getCookie("avatar") == "1") {
        _this.$router.push({
          path: "/repairconfirm",
