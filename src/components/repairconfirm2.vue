<template>
  <div style="position:relative" class="xiongxiong12">
    ...
    <ul class="main">
      ...
      <!-- @click="customer" -->
      <li v-show="isyuangong1">
        <span>客户：{{this.$store.state.repairorder.name}}</span>
        <!-- <i></i> -->
      </li>
      <li v-show="isyuangong1">
        <span>终端客户级别：</span>
        <el-select v-model="this.$store.state.repairorder.customer_level_id" clearable disabled>
          <el-option
    ...
    <div v-show="this.$store.state.repairorder.ApplicationType =='维修'">
      <div v-show="this.$store.state.repairorder.image !=''">
        <img
          ...
          style="  width: 2.6rem; margin: 0.4rem; height: 2.6rem; display: inline-block; vertical-align: top;"/>
      </div>
      <video
...
export default {
  ...
  methods: {
    ...
    send() {
      ...
      this.$Axios
        .post(baseUrl + "/applyJob/updateApplyJobService",response.data.data.groupId)
        .then(function(response) {
