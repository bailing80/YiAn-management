<template>
  <div style="position:relative" class="xiongxiong12">
    ...
    <ul class="main">
      <li>
        <p>申请类型：{{this.$route.query.leixing}}</p>
      </li>
      <li v-for="(list,index) in this.$route.query.list" :key="index">
      ...
      <!-- @click="customer" -->
      <li v-show="isyuangong1" v-for="(list,index) in this.$route.query.list" :key="index">
      ...
      >
        <p>配件名称：{{accessories}}</p>
      </li>
      <li
        v-show="bool01 == 1 && isyuangong && this.$store.state.repairorder.ApplicationType == '维修'"
      >
        <p>物料编号：{{MaterialCode}}</p>
      </li>
      ...
      <li v-show="isyuangong & this.$store.state.repairorder.ApplicationType =='维修'">
        <u>解决方法:</u>
        <s>{{solution}}</s>
      </li>
    </ul>
    ...
    <div class="sendbtn mb1" @click="send">提交</div>
...
export default {
 ...
 this.$Axios
  ...
  methods: {
    ...
    toback() {
      if (Cookie.getCookie("avatar") == 2) {
        this.$router.push({ path: "/chongxintibao" });
      } else if (Cookie.getCookie("avatar") == "1") {
        this.$router.push({ path: "/chongxintibao" });
      } else if (Cookie.getCookie("avatar") == "3") {
        this.$router.push({ path: "/chongxintibao" });
    ...
    //   console.log(repairorder.ApplicationType+"安装类型1")
    //   var data = {
    //     deviceId: "", //设备号
    ...
    //     ApplicationType:repairorder.ApplicationType,
    ...
    send() {
      ...
      var data = {
        userId: "",
        deviceId: "", //设备号
        customerId: this.$route.query.id2, //客户id
        customerName: repairorder.name, //客户名称  （）医院名称
        customerLevel: repairorder.customer_level_id, //医院等级
        contacts: repairorder.contact, //联系人
      ...
      let letgroupId=this.$route.query.groupId
      this.$Axios
        .post(baseUrl + "/applyJob/updateApplyJobService", {groupId:letgroupId})
        .then(function(response) {
