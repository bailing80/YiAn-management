<template>
  <div style="position:relative" class="xiongxiong1">
    ...
    <ul class="main">
      ...
      <li @click="tofault" v-show="this.$store.state.repairorder.ApplicationType =='维修' ">
      ...
      <li
        @click="xuanzhe(3)"
        v-show="isyuangong && this.$route.query.leixing =='维修' "
      >
        <p>是否需要更改配件</p>
        <i></i>
      </li>
      <li
        v-show="bool01 == 1 && isyuangong && this.$route.query.leixing =='维修'"
      >
        <span>配件名称：</span>
      ...
      <li v-show="this.$route.query.leixing =='维修'">
      ...
      <li v-show="this.$route.query.leixing =='维修' && isyuangong">
    ...
    <div
      ...
      v-show="this.$route.query.leixing =='维修'"
    >附带机器故障图片，最多上传三张故障图片，且大小不能超过8M</div>
    <div style="margin:.4rem .4rem" v-show="this.$route.query.leixing =='维修'">
      <el-upload
        :multiple="multiple"
        :headers="{token:token}"
...
//——————————————————————————————————————————————
let letgroupId=this.$route.query.id;
 this.$Axios
      .post(
        baseUrl +
          "/applyJob/selectApplyJobAndProduct",
          {groupId: letgroupId}
      )
      .then(function(response) {
       let DeviceNumber = response.data.data.yianAppJobEntity.deviceId
        ...
        .then(res => {
          if (res.data.code == 200) {
            ...
            if (Cookie.getCookie("avatar") == 2) {
              _this.$router.push({
                path: "/repairconfirm2",
                query: { id: 2, leixing:this.$route.query.leixing}
              });
            } else if (Cookie.getCookie("avatar") == "1") {
              _this.$router.push({
                path: "/repairconfirm2",
    ...
    totosend() {
      ...
      if (Cookie.getCookie("avatar") == 2) {
        _this.$router.push({
          ...
          query: { id: 2,leixing:this.$route.query.leixing,groupId:this.$route.query.id}
        });
      } else if (Cookie.getCookie("avatar") == "1") {
        _this.$router.push({
          path: "/repairconfirm2",
