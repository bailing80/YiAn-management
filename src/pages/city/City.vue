...
export default {
  ...
  watch: {
    ...
    conit() {
      ...
      _this.pagequery.city = this.conit;

      //医院查询  // .post(baseUrl + "/customerAdmin/queryCustomerList", {
      this.$Axios
        .post(
          baseUrl + "/customerAdmin/queryCustomerListForHisp",
          _this.pagequery
        )
        .then(function(response) {
          //重复点击
          _this.cities = [];
          if (response.data.code == 200) {
            _this.pagequery.total = response.data.data.total;
  ...
  methods: {
    isyiyuan(userType) {
      ...
      this.pagequery.userType = userType;
      //医院查询
      _this.$Axios
        .post(
          baseUrl + "/customerAdmin/queryCustomerListForHisp",
    ...
    loadMore() {
      if (this.boolershow) {
        if (
          Math.ceil(this.pagequery.total / this.pagequery.rows) >
          this.pagequery.page
        ) {
    ...
    search() {
      ...
      this.pagequery.customerName  = this.context
